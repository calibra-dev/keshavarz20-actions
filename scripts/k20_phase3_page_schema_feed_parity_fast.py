...
import concurrent.futures, datetime, html as htmlmod, json, os, re, time
...
UA={"User-Agent":"k20-phase3-feed-parity-fast/1.0","Cache-Control":"no-cache","Accept":"application/json"}

def api_get(path, params=None):
    last=None
    for attempt in range(3):
        try:
            r=requests.get(urljoin(BASE,path.lstrip("/")),params=params,auth=AUTH,timeout=45,headers=UA)
            r.raise_for_status(); return r
        except Exception as e:
            last=e; time.sleep(attempt+1)
    raise last
...
def setting_value(group,key):
    try:
        rows=api_get(f"wp-json/wc/v3/settings/{group}").json()
        for x in rows:
...
def process(item):
    ...
    p=api_get(f"wp-json/wc/v3/products/{pid}").json()
    public=public_get(p.get("permalink"))
    plist=extract_products(public.text[:1500000])
    sku=(p.get("sku") or "").strip()
    schema=None
    for z in plist:
        if sku and str(z.get("sku") or "").strip()==sku:
            schema=z;break
    ...
    price_parity="not_evaluated";expected_schema_price=None
    if raw_price is not None and schema_price is not None:
        cur=str(currency).upper();sc=str(schema_currency or "").upper()
        if cur==sc:
        ...
        else:price_parity="currency_rule_unknown"
    schema_sku=(schema or {}).get("sku");schema_name=(schema or {}).get("name")
    ...
    feed={
      "id":sku or f"K20-{pid}","wp_product_id":pid,"title":p.get("name"),"link":p.get("permalink"),
      "image_link":((p.get("images") or [{}])[0].get("src") if p.get("images") else None),
      "availability":feed_avail,"store_price_raw":p.get("price"),"store_currency":currency,
      ...
      "feed_scope":"internal_canonical_top30","not_submitted_to_external_merchant":True
    }
    checks={
      "product_schema_present":bool(schema),
      "sku_match":bool(schema) and str(schema_sku or "").strip()==sku,
      "name_present":bool(schema_name),"offer_present":bool(off),
    ...
    return item["rank"],feed,parity

items=pim.get("products",[])
rows=[]
with concurrent.futures.ThreadPoolExecutor(max_workers=min(8,max(1,len(items)))) as ex:
    futs=[ex.submit(process,item) for item in items]
    for f in concurrent.futures.as_completed(futs):
        rows.append(f.result())
rows.sort(key=lambda x:x[0])
feed=[x[1] for x in rows];parity=[x[2] for x in rows]
summary={
 ...
 "price_mismatch":sum(1 for x in parity if x["checks"]["price_parity"]=="fail"),"store_currency":currency
}
Path("phase3-results").mkdir(exist_ok=True)
json.dump({"ok":True,"version":"phase3-top30-internal-feed-v1","generated_at_utc":datetime.datetime.now(datetime.timezone.utc).isoformat(),
...
json.dump({"ok":True,"version":"phase3-page-schema-feed-parity-fast-v1","summary":summary,"products":parity},
          open("phase3-results/page-schema-feed-parity.json","w",encoding="utf-8"),ensure_ascii=False,indent=2)
print("PHASE3_FEED_PARITY_FAST_OK",summary)
