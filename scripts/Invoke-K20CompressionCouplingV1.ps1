...
$maxRelated = if ($request.max_related) { [int]$request.max_related } else { 8 }
if ($maxRelated -lt 3 -or $maxRelated -gt 8) { Fail 'max_related must be 3..8' }

$tc = @(Invoke-K20 'GET' ("wp-json/wc/v3/products/categories?slug=" + [Uri]::EscapeDataString($targetSlug) + "&per_page=100"))
if ($tc.Count -ne 1) { Fail "Expected one target category; got $($tc.Count)" }
$targetCategory = $tc[0]
$targetCategoryId = [int]$targetCategory.id
$pc = @(Invoke-K20 'GET' ("wp-json/wc/v3/products/categories?slug=" + [Uri]::EscapeDataString($pipeSlug) + "&per_page=100"))
if ($pc.Count -ne 1) { Fail "Expected one pipe category; got $($pc.Count)" }
$pipeCategory = $pc[0]
$pipeCategoryId = [int]$pipeCategory.id
...
if ($mode -eq 'apply_description') {
  foreach ($t in $targets) {
    ...
    $beforeOutside = Get-PlainText (Remove-RecommendationBlock $beforeDescription)
    $desired = Set-RecommendationBlock $beforeDescription (Build-RecommendationBlock $t)
    $current = Test-RecommendationReadback $beforeDescription $t
    if ($current.ok) {
      $changes += [pscustomobject]@{id=$id;name=$t.name;changed=$false;semantic_verified=$true;integrity_unchanged=$true;outside_text_unchanged=$true;reason='already_current'}
      continue
    }
    $null = Invoke-K20 'PUT' "wp-json/wc/v3/products/$id" ([ordered]@{description=$desired})
    $after = Invoke-K20 'GET' "wp-json/wc/v3/products/$id"
    $check = Test-RecommendationReadback ([string]$after.description) $t
    $integrity = ((Get-IntegritySnapshot $after) -eq $beforeIntegrity)
    $outside = ((Get-PlainText (Remove-RecommendationBlock ([string]$after.description))) -ceq $beforeOutside)
    if (-not $check.ok) { Fail "Recommendation readback failed for ${id}: $($check.reason)" }
    if (-not $integrity) { Fail "Protected-field integrity mismatch for $id" }
    if (-not $outside) { Fail "Text outside recommendation block changed for $id" }
...
$record = [ordered]@{
  ...
  target_category=[ordered]@{id=$targetCategoryId;name=[string]$targetCategory.name;slug=[string]$targetCategory.slug;count=[int]$targetCategory.count;description=[string]$targetCategory.description}
  ...
  ambiguous_target_count=$unparsed.Count
  missing_pipe_target_count=$missingPipe.Count
  empty_recommendation_count=$empty.Count
  rules=[ordered]@{equal='one nominal size: exact-size PE pipe plus useful same-size fittings';reducer='two nominal sizes: pipe for each side plus equal couplings and side-specific fittings';pipe='must belong to live polyethylene-pipe category';stale='hidden and __trashed products are excluded';mutation='description field only';protected='price, regular/sale price, stock, SKU, name, slug, short description, taxonomy, images, attributes, upsells and cross-sells are readback-protected'}
  ambiguous_targets=@($unparsed)
  missing_pipe_targets=@($missingPipe | Select-Object id,name,kind,sizes_mm,missing_pipe_sizes)
