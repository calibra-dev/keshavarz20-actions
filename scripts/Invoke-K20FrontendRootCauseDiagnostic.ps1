...
if($phrases.Count -lt 1){throw 'At least one phrase is required.'}
if($urls.Count -lt 1){throw 'At least one URL is required.'}
  ...
  if($Node-is[pscustomobject]){
    ...
    if($typeProp){
      ...
      if($types -contains 'Organization'){
        $logo=$Node.PSObject.Properties['logo'];$name=$Node.PSObject.Properties['name'];$url=$Node.PSObject.Properties['url'];
        $Organizations.Add([pscustomobject][ordered]@{name=if($name){$name.Value}else{$null};url=if($url){$url.Value}else{$null};has_logo=($null-ne$logo -and $null-ne$logo.Value);logo=if($logo){$logo.Value}else{$null}})
      }
    }
    foreach($p in $Node.PSObject.Properties){Walk-Json $p.Value $Organizations}
    return
  }
  if(($Node-is[System.Collections.IEnumerable])-and-not($Node-is[string])){foreach($item in $Node){Walk-Json $item $Organizations}}
}
$pageMatches=New-Object System.Collections.Generic.List[object]
$searchMatches=New-Object System.Collections.Generic.List[object]
foreach($phrase in $phrases){
  $enc=[Uri]::EscapeDataString($phrase)
  try{
    $pages=@(Invoke-JsonGet "wp-json/wp/v2/pages?context=edit&search=$enc&per_page=100&_fields=id,slug,status,link,parent,template,title,content,excerpt")
    foreach($p in $pages){
      $raw=[string]$p.content.raw;$title=[string]$p.title.raw;$plain=TextOnly $raw
      $pageMatches.Add([pscustomobject][ordered]@{phrase=$phrase;id=[int]$p.id;slug=[string]$p.slug;status=[string]$p.status;link=[string]$p.link;parent=[int]$p.parent;template=[string]$p.template;title=$title;content_contains_phrase=$raw.Contains($phrase);title_contains_phrase=$title.Contains($phrase);content_snippet=Snippet $plain $phrase})
    }
  }catch{$pageMatches.Add([pscustomobject][ordered]@{phrase=$phrase;error=$_.Exception.Message})}
  try{
    $rows=@(Invoke-JsonGet "wp-json/wp/v2/search?search=$enc&per_page=100&type=post&subtype=any")
    foreach($r in $rows){$searchMatches.Add([pscustomobject][ordered]@{phrase=$phrase;id=[int]$r.id;title=[string]$r.title;url=[string]$r.url;type=[string]$r.type;subtype=[string]$r.subtype})}
  }catch{$searchMatches.Add([pscustomobject][ordered]@{phrase=$phrase;error=$_.Exception.Message})}
}
$frontends=New-Object System.Collections.Generic.List[object]
foreach($url in $urls){
  try{
    $resp=Invoke-WebRequest -Uri $url -Method GET -Headers @{'User-Agent'='K20-Frontend-RootCause/2026-09-16';'Accept'='text/html,application/xhtml+xml'} -MaximumRedirection 5 -TimeoutSec 90 -SkipHttpErrorCheck
    $html=[string]$resp.Content
    $h1s=@();foreach($m in [regex]::Matches($html,'(?is)<h1\b[^>]*>(.*?)</h1>')){$h1s+=TextOnly $m.Groups[1].Value}
    $missingAlt=New-Object System.Collections.Generic.List[object]
    foreach($m in [regex]::Matches($html,'(?is)<img\b[^>]*>')){
      $tag=$m.Value;$altPresent=[regex]::IsMatch($tag,'(?is)\balt\s*=');$alt=Get-Attr $tag 'alt'
      if((-not$altPresent)-or[string]::IsNullOrWhiteSpace($alt)){
        $src=Get-Attr $tag 'src';if([string]::IsNullOrWhiteSpace($src)){$src=Get-Attr $tag 'data-src'}
        $cls=Get-Attr $tag 'class';$width=Get-Attr $tag 'width';$height=Get-Attr $tag 'height'
        $missingAlt.Add([pscustomobject][ordered]@{alt_attribute_present=$altPresent;src=$src;class=$cls;width=$width;height=$height;tag_excerpt=if($tag.Length-gt420){$tag.Substring(0,420)}else{$tag}})
      }
    }
    $orgs=New-Object System.Collections.Generic.List[object]
