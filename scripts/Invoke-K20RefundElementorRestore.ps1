...
$revision=Invoke-RestMethod -Uri "$base/wp-json/wp/v2/pages/${id}/revisions/${rev}?context=edit&_fields=id,meta" -Headers $headers -Method GET -TimeoutSec 180
...
$write=Invoke-RestMethod -Uri "$base/wp-json/wp/v2/pages/${id}?context=edit&_fields=id,meta" -Headers $headers -Method POST -ContentType 'application/json; charset=utf-8' -Body $body -TimeoutSec 180
$writeStored=[string]$write.meta._elementor_data
if((Hash $writeStored)-ne$expected){throw 'Write-response hash guard failed.'}
$check=Invoke-RestMethod -Uri "$base/wp-json/wp/v2/pages/${id}?context=edit&_fields=id,meta" -Headers $headers -Method GET -TimeoutSec 180
