...
function Get-UseText([string]$Family,[string]$SizeText){
  switch($Family){
    ...
    'clamp_single_cap'{return "برای مهار و نگه‌داشتن اتصال شیلنگی هم‌سایز $SizeText به کار می‌رود. در این مدل، تطبیق قطر شیلنگ، قطعه داخل شیلنگ و نحوه قرارگیری کپه قبل از سفت‌کردن اهمیت دارد."}
    'clamp_double_cap'{return "برای مهار اتصال شیلنگی هم‌سایز $SizeText با ساختار دوکپه استفاده می‌شود. دو نیمه باید یکنواخت و بدون کجی روی مجموعه قرار بگیرند تا فشار موضعی روی شیلنگ ایجاد نشود."}
    ...
    'valve_iron'{return "سوپاپ آهنی $SizeText برای استفاده در مسیر آب و تجهیزات هم‌سایز انتخاب می‌شود. جهت نصب و وضعیت باز و بسته‌شدن قطعه باید مطابق ساختار و علامت‌های روی بدنه همان محصول انجام شود."}
...
function Get-BuyChecks([string]$Family){
  switch -Regex($Family){
    '^clamp_|^hose_tail$|pressed_hose_adapter'{return @('قطر واقعی شیلنگ را با سایز محصول تطبیق دهید.','نوع قطعه‌ای که داخل یا مقابل شیلنگ قرار می‌گیرد مشخص باشد.','بست یا روش مهار را متناسب با ساختار همان اتصال انتخاب کنید.','پس از نصب، اتصال را از نظر لغزش شیلنگ و نشتی کنترل کنید.')}
    '^valve_'{return @('سایز دهانه و نوع اتصال دو طرف را با مسیر تطبیق دهید.','جهت نصب را از روی ساختار یا علامت روی بدنه همان قطعه رعایت کنید.','برای اتصال رزوه‌ای یا فلنجی، روش آب‌بندی مناسب همان اتصال را استفاده کنید.','پس از آبگیری، عملکرد قطعه و نبود نشتی را کنترل کنید.')}
    'flange|flanged'{return @('سایز اسمی هر فلنج را با قطعه مقابل تطبیق دهید.','الگوی سوراخ پیچ و سطح نشیمن دو فلنج باید سازگار باشد.','واشر مناسب بین سطوح فلنجی قرار گیرد.','پیچ‌ها را به‌صورت متقاطع و یکنواخت سفت کنید و از کشیدن لوله با پیچ‌ها خودداری کنید.')}
    default{return @('سایز و نوع اتصال را با قطعه مقابل تطبیق دهید.','مسیر نصب را بدون تنش و کجی آماده کنید.','آب‌بندی متناسب با نوع اتصال انجام شود.','بعد از راه‌اندازی محل اتصال را از نظر نشتی کنترل کنید.')}
  }
}
...
function Get-Related($Target){
  $families=@(Get-RelatedFamilies ([string]$Target.family));if($families.Count-eq0){return @()}
  ...
  foreach($family in $families){
    foreach($candidate in $targets){
      ...
      $common=$false
      foreach($a in $sizes){foreach($b in @($candidate.sizes_inch)){if([double]$a-eq[double]$b){$common=$true;break}};if($common){break}}
...
if($mode-eq'apply_content'){
  ...
  foreach($t in $targets){
    $before=Invoke-K20 'GET' "wp-json/wc/v3/products/$([int]$t.id)"
    $integrityBefore=Get-IntegritySnapshot $before
    ...
    if(-not$already){$null=Invoke-K20 'PUT' "wp-json/wc/v3/products/$([int]$t.id)" ([ordered]@{short_description=$desiredShort;description=$desiredDescription})}
    $after=Invoke-K20 'GET' "wp-json/wc/v3/products/$([int]$t.id)"
    $semantic=(([string]$after.short_description)-ceq$desiredShort)-and(([string]$after.description)-ceq$desiredDescription)
    $integrity=((Get-IntegritySnapshot $after)-ceq$integrityBefore)
    if(-not$semantic){Fail "Content readback failed for $([int]$t.id)"}
    if(-not$integrity){Fail "Protected-field integrity mismatch for $([int]$t.id)"}
    $changes+=[ordered]@{id=[int]$t.id;name=[string]$t.name;family=[string]$t.family;changed=(-not$already);semantic_verified=$true;integrity_unchanged=$true;reason=if($already){'already_current'}else{'written_and_verified'}}
  }
  if($updateCategory){
