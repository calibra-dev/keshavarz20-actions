...
def main(outpath):
    m=load("phase18/automation-governance.json"); p=load("automation-policy/seo-god-2026.json"); q=load("question-engine/config.json"); probe=load("phase18-results/news-read-probe.json")
    ...
    add("news_custom_type_read_probe",probe.get("ok") is True and probe.get("read_only") is True and probe.get("writes_performed")==0); add("news_no_broken_rest_cpt_route","/wp-json/wp/v2/news" not in news_base and "wp.getPosts" in news_base)
    ...
    result={"phase":18,"status":"PASS" if passed else "FAIL","checks":checks,"acceptance":{"news_0800_architecture_current_and_tested":all(x["pass"] for x in checks if x["name"] in ["news_schedule_0800_tehran","news_api_generator_manual_fallback_only","news_queue_push_main_only","news_pr_does_not_publish","news_global_concurrency_lock","news_custom_type_read_probe","news_no_broken_rest_cpt_route"]),"article_schedule_validated":all(x["pass"] for x in checks if x["name"].startswith("article_")),"question_cadence_truthful":all(x["pass"] for x in checks if x["name"].startswith("question_")),"skip_day_supported":all(x["pass"] for x in checks if "skip_day" in x["name"]),"duplicate_drafts_guarded":all(x["pass"] for x in checks if "duplicate" in x["name"])},"runtime_evidence":m["observed_runtime"],"news_read_probe":probe,"site_mutations_by_acceptance_workflow":0,"commerce_mutations":0,"secret_values_persisted":False}
