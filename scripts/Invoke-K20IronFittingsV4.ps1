...
function Normalize-Html([string]$Html){
  ...
  $x=$Html.Replace("`r`n","`n").Replace("`r","`n")
  $x=[regex]::Replace($x,'>\s+<','><')
  return $x.Trim()
}
...
function Get-IntegritySnapshot($P){
  $o=[ordered]@{
    name=[string]$P.name;slug=[string]$P.slug;status=[string]$P.status;type=[string]$P.type;sku=[string]$P.sku;
    price=[string]$P.price;regular_price=[string]$P.regular_price;sale_price=[string]$P.sale_price;
    catalog_visibility=[string]$P.catalog_visibility;featured=[bool]$P.featured;virtual=[bool]$P.virtual;downloadable=[bool]$P.downloadable;
    manage_stock=[bool]$P.manage_stock;stock_quantity=$P.stock_quantity;stock_status=[string]$P.stock_status;backorders=[string]$P.backorders;sold_individually=[bool]$P.sold_individually;
    weight=[string]$P.weight;dimensions=$P.dimensions;shipping_class=[string]$P.shipping_class;reviews_allowed=[bool]$P.reviews_allowed;purchase_note=[string]$P.purchase_note;menu_order=[int]$P.menu_order;
...
if($mode-eq'apply_content'){
  ...
  foreach($t in $targets){
    ...
    $already=((Normalize-Html ([string]$before.short_description))-ceq(Normalize-Html $desiredShort))-and((Normalize-Html ([string]$before.description))-ceq(Normalize-Html $desiredDescription))
    if(-not$already){$updates+=[ordered]@{id=$id;short_description=$desiredShort;description=$desiredDescription}}
  }

  if($updates.Count-gt0){
    $null=Invoke-K20 'POST' 'wp-json/wc/v3/products/batch' ([ordered]@{update=@($updates)})
  }
  ...
  foreach($t in $targets){
    ...
    $semantic=((Normalize-Html ([string]$after.short_description))-ceq(Normalize-Html ([string]$desired.short)))-and((Normalize-Html ([string]$after.description))-ceq(Normalize-Html ([string]$desired.description)))
    ...
    $changed= -not(((Normalize-Html ([string]$before.short_description))-ceq(Normalize-Html ([string]$desired.short)))-and((Normalize-Html ([string]$before.description))-ceq(Normalize-Html ([string]$desired.description))))
  ...
  if($updateCategory){
    ...
    $catSnapshot=([ordered]@{id=[int]$catBefore.id;name=[string]$catBefore.name;slug=[string]$catBefore.slug;parent=[int]$catBefore.parent;count=[int]$catBefore.count;display=[string]$catBefore.display;image=$catBefore.image}|ConvertTo-Json -Depth 10 -Compress)
    $desiredCategory=Build-CategoryDescription
    $catAlready=(Normalize-Html ([string]$catBefore.description))-ceq(Normalize-Html $desiredCategory)
    ...
    $catSemantic=(Normalize-Html ([string]$catAfter.description))-ceq(Normalize-Html $desiredCategory)
    $catIntegrity=(([ordered]@{id=[int]$catAfter.id;name=[string]$catAfter.name;slug=[string]$catAfter.slug;parent=[int]$catAfter.parent;count=[int]$catAfter.count;display=[string]$catAfter.display;image=$catAfter.image}|ConvertTo-Json -Depth 10 -Compress)-ceq$catSnapshot)
...
$record=[ordered]@{
  ...
  requested_update_count=if($mode-eq'apply_content'){$updates.Count}else{0};
...
Write-Host "IRON_FITTINGS_OK mode=$mode products=$($targets.Count) unknown=$($unknown.Count) updates=$(if($mode-eq'apply_content'){$updates.Count}else{0}) changed=$(@($changes|Where-Object{$_.changed}).Count)"
