...
require_once __DIR__.'/includes/class-k20-bridge-v32-snapshots.php';
require_once __DIR__.'/includes/class-k20-bridge-v32-approval.php';
require_once __DIR__.'/includes/class-k20-bridge-v32-content.php';
require_once __DIR__.'/includes/class-k20-bridge-v32-media.php';
...
final class K20_Bridge_V3 {
    ...
    public static function health(): WP_REST_Response {
        ...
        return self::response([
            'ok'=>true,'status'=>'healthy','plugin'=>'Keshavarz20 Bridge v3','version'=>self::VERSION,'contract'=>self::CONTRACT,
            'wp_version'=>get_bloginfo('version'),'php_version'=>PHP_VERSION,'https'=>is_ssl(),
            'woocommerce'=>class_exists('WooCommerce'),'yoast'=>defined('WPSEO_VERSION'),'elementor'=>defined('ELEMENTOR_VERSION'),
            'object_cache'=>wp_using_ext_object_cache(),
            'media_engine'=>class_exists('K20_Bridge_V32_Media'),
            'content_engine'=>class_exists('K20_Bridge_V32_Content'),
            'job_engine'=>class_exists('K20_Bridge_V32_Jobs'),
            'rollback_engine'=>class_exists('K20_Bridge_V32_Snapshots'),
    ...
    public static function capabilities_route(): WP_REST_Response {
        return self::response([
            'ok'=>true,'version'=>self::VERSION,'contract'=>self::CONTRACT,'actions'=>self::actions(),'rest_routes'=>self::route_patterns(),
    ...
    public static function execute(WP_REST_Request $request): WP_REST_Response {
        ...
        if ($action!=='approval.status' && $action!=='approval.execute' && K20_Bridge_V32_Approval::requires($action,$body)) {
            $gate=K20_Bridge_V32_Approval::gate($action,$body);
            if (is_wp_error($gate)) return self::wp_error_response($gate);
        }
        ...
        try {
            ...
            $envelope=[
                'ok'=>true,'bridge'=>'Keshavarz20 Bridge v3','version'=>self::VERSION,'contract'=>self::CONTRACT,
                'action'=>$action,'request_id'=>$request_id?:null,'dry_run'=>$dry,'executed_at_utc'=>gmdate('c'),'result'=>self::redact($result)
            ];
    ...
    public static function dispatch_for_job(string $action,array $body,bool $dry) {
        ...
        switch ($action) {
            ...
            case 'content.patch': return K20_Bridge_V32_Content::patch($body,$dry);
            case 'content.block.inspect': return K20_Bridge_V32_Content::block_inspect($body);
            case 'content.block.patch': return K20_Bridge_V32_Content::block_patch($body,$dry);
            ...
            case 'elementor.edit': return K20_Bridge_V32_Content::elementor_edit($body,$dry);
            case 'elementor.structure': return K20_Bridge_V32_Content::elementor_structure($body,$dry);

            case 'media.import': return K20_Bridge_V32_Media::import_url($body,$dry);
            case 'media.transform': return K20_Bridge_V32_Media::transform($body,$dry);
            case 'media.metadata': return K20_Bridge_V32_Media::metadata($body,$dry);
            case 'media.hash': return K20_Bridge_V32_Media::hash($body,$dry);
            case 'media.duplicates': return K20_Bridge_V32_Media::duplicates($body);
            case 'media.optimize': return K20_Bridge_V32_Media::optimize($body,$dry);
            ...
            case 'job.create': return K20_Bridge_V32_Jobs::create($body,$dry);
            case 'job.status': return K20_Bridge_V32_Jobs::status($body);
            case 'job.run': return K20_Bridge_V32_Jobs::run($body,$dry,[__CLASS__,'dispatch_for_job']);
            case 'job.retry_failed': return K20_Bridge_V32_Jobs::retry_failed($body,$dry);

            case 'snapshot.list': return K20_Bridge_V32_Snapshots::list($body);
            case 'snapshot.rollback': return K20_Bridge_V32_Snapshots::rollback($body,$dry);

            case 'approval.status': return K20_Bridge_V32_Approval::status($body);
            case 'approval.execute': return K20_Bridge_V32_Approval::execute($body,$dry,[__CLASS__,'dispatch_for_job']);

            case 'observability.status': return K20_Bridge_V32_Observability::status();
            case 'selftest.run': return K20_Bridge_V32_Observability::selftest($body,$dry,[__CLASS__,'dispatch_for_job']);

            case 'update.check': return K20_Bridge_V32_Updater::check();
            case 'update.stage': return K20_Bridge_V32_Updater::stage($body,$dry);
            case 'update.apply': return K20_Bridge_V32_Updater::apply($body,$dry);
            case 'update.rollback': return K20_Bridge_V32_Updater::rollback($body,$dry);
        }
    ...
    private static function rest_proxy(array $body,bool $dry) {
        $method=strtoupper((string)($body['method']??'GET')); $path='/'.ltrim((string)($body['path']??''),'/');
        $query=is_array($body['query']??null)?$body['query']:[]; $payload=is_array($body['payload']??null)?$body['payload']:[];
        ...
        $req=new WP_REST_Request($method,$path); if ($query) $req->set_query_params($query); if ($payload) $req->set_body_params($payload);
        $res=rest_do_request($req); if (is_wp_error($res)) return $res;
    ...
    private static function system_info(): array {
        ...
        return [
            ...
            'active_plugins'=>$rows,'woocommerce'=>class_exists('WooCommerce'),
            'yoast'=>defined('WPSEO_VERSION')?WPSEO_VERSION:null,'elementor'=>defined('ELEMENTOR_VERSION')?ELEMENTOR_VERSION:null,
            'object_cache'=>wp_using_ext_object_cache()
        ];
    }
    ...
    private static function seo_update(int $id,array $payload,bool $dry) {
        ...
        $before=[]; foreach (array_keys($changes) as $m) $before[$m]=get_post_meta($id,$m,true);
        K20_Bridge_V32_Snapshots::capture('seo_meta',$id,'yoast',$before,['action'=>'seo.update']);
        foreach ($changes as $m=>$v) update_post_meta($id,$m,$v);
    ...
    private static function batch(array $payload,bool $outer_dry) {
        ...
        foreach ($items as $i=>$item) {
            ...
            if (in_array($action,['batch','job.create','job.run','approval.execute','update.apply','update.rollback'],true) || !in_array($action,self::actions(),true)) {
                $out[]=['index'=>$i,'ok'=>false,'error'=>'action_not_allowed']; continue;
            }
    ...
    private static function audit(string $action,bool $ok,bool $dry,string $request_id,string $status): void {
        ...
        $rows[]=['at_utc'=>gmdate('c'),'action'=>$action,'ok'=>$ok,'dry_run'=>$dry,'request_id'=>$request_id?:null,'status'=>$status];
    ...
    private static function blocked_path($value,string $path=''): ?string {
        ...
        foreach ($value as $k=>$v) {
            ...
            if (is_array($v) && ($f=self::blocked_path($v,$full))!==null) return $f;
        }
        return null;
    }
    ...
    private static function wp_error_response(WP_Error $e): WP_REST_Response {
        $data=$e->get_error_data(); $status=is_array($data)&&isset($data['status'])?(int)$data['status']:400;
        $extra=is_array($data)?$data:[]; unset($extra['status']);
        return self::error($e->get_error_code(),$e->get_error_message(),$status,$extra);
    }
    private static function error(string $code,string $message,int $status,array $extra=[]): WP_REST_Response {
        return self::response(array_merge(['ok'=>false,'bridge'=>'Keshavarz20 Bridge v3','version'=>self::VERSION,'contract'=>self::CONTRACT,'code'=>$code,'message'=>$message],$extra),$status);
    }
}
K20_Bridge_V3::boot();
